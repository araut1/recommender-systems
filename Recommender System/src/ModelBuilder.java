 ...
 * 
 * Associated with model development and missing values prediction
 */
...
{	
	...
	/**
	 * Finds the cosine similarity between two items to be compared
	 * 
	 * @param input
	 * @param average
	 * @param rows
	...
	{		
		...
		for(int i = 1; i < cols; i++) 
		{
			for(int j = 1; j < cols; j++) 
			{
				...
				{					
					//Check if both the items are rated by the user or not
				...
				// Applies adjacent cosine similarity formula
				isSimilar = dotProduct/(Math.sqrt(normA) * Math.sqrt(normB));
				
				//Checks if similarity is greater than the threshold value
				if(isSimilar >= 0.3) 
				{
					addItem(map, i, j);
				}
				
				//Adds value to the similarity matrix
	...
	/**
	 * 
	 * Finds the prediction for a user i and a item j
	 * 
	 * @param input
	 * @param output
	 * @param rows
	...
	{		
		...
		for (int i = 1; i < rows; i++) 
		{
			for (int j = 1; j < cols; j++) 
			{				
				//Predicts the value only in case of zero. Otherwise, assign the input value as prediction not required
				if(input[i][j] == 0) 
				{					
					// Gets the similar items for the item j
					list = map.get(j);
					
					// Checks item list is null and if it is assign 1 
					if (list != null) 
					{
						missingValue = calculateWeightedSum(input, i, j, list);
						finalValue = (int) Math.round(missingValue);
						
						//Round up the predicted values (predictited valaues are rounded to integer)
						if (finalValue < 1) 
						{
							output[i][j] = 1;
						}
